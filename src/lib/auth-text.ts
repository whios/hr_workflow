 ...
 */
export function buildAuthorizationText(companyName: string, candidateName: string, idMasked: string): string {
  const scopeList = AUTH_SCOPE_ITEMS.map((item, i) => `${i + 1}. ${item}`).join('；');

  return [
    `本人 ${candidateName}（身份证号：${idMasked}）确认与 ${companyName} 存在入职/合作背景调查关系，特此作出以下授权与声明。`,
    '',
    '1 信息真实性承诺',
    ...
    '敏感信息处理：本人同意完整身份证号随本授权回执保存，并仅供指定 HR 进行身份核验；回执链接不得向无关人员转发。',
    '',
    '4 授权期限与撤回限制',
